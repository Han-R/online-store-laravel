@extends('layouts.cp')
@section('title') {{ucwords(__('cp.edit_quantity'))}}
@endsection
@section('css')
@endsection
@section('content')
                ...
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <span class="caption-subject font-dark sbold uppercase"
                              style="color: #e02222 !important;">{{__('cp.edit_quantity')}}</span>
                    </div>
                </div>
                <div class="portlet-body form">
                    <label style="font-weight: bolder;font-size: 20px;">{{ucwords(__('cp.product_name'))}} : {{$product->name}}</label>
                    <form method="post" action="{{url(app()->getLocale().'/admin/editQuantity/'.$item->id)}}"
                        ...
                        <div class="form-body">
                            ...
                            <fieldset>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6">
                                        <input type="text" class="form-control" value="{{$item->price}}" placeholder="{{__('common.price')}}" name="price" required>
                                    </div>
                                </div>
                            </fieldset>
                            <fieldset>
                                    ...
                                    <div class="col-md-6">
                                        <select name="supplier_id" class="form-control" required>
                                            ...
                                            @foreach($suppliers as $one)
                                                <option value="{{$one->id}}" @if($item->supplier_id == $one->id) selected @endif>{{$one->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
