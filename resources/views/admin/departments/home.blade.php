@extends('layouts.cp')
@section('title') {{__('cp.departments')}}
@endsection
@section('css')
@endsection
@section('content')
        ...
        <div class="portlet-body">
            <div class="table-toolbar">
                <div class="row">
                    <div class="col-sm-9">
                        <div class="btn-group">
                            <a href="{{url(getLocal().'/admin/department/create')}}" style="margin-right: 5px"
                ...
                <div class="box-filter-collapse">
                    <form class="form-horizontal" method="get" action="{{url(getLocal().'/admin/departments')}}">
                        <div class="row">
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                <div class="row">
                                    <div class="col-md-offset-3 col-md-9">
                                        ...
                                        <a href="{{url('admin/categories')}}" type="submit" class="btn sbold btn-default ">{{__('common.reset')}}
            ...
            <input type="hidden" id="url" value="{{url("/en/admin/changeStatus/departments")}}">
            <table class="table table-striped table-bordered table-hover table-checkable order-column" id="toolsTable">
                <thead>
                <tr>
                    <th></th>
                    <th> {{ucwords(__('common.title'))}}</th>
                    <th> {{ucwords(__('common.icon'))}}</th>
                ...
                @forelse($items as $item)
                    <tr class="odd gradeX" id="tr-{{$item->id}}">
                        ...
                        <td> {{$item->name}}</td>
                        <td>@if($item->icon)<img src="{{url($item->icon)}}" width="50px" height="50px">@else {{__('common.no_icon')}} @endif
                        </td>
                        <td class="center">{{$item->created_at}}</td>
                        <td>
                            <div class="btn-group btn-action">
                                <a href="{{url(getLocal().'/admin/department/'.$item->id.'/edit')}}"
...
@section('script')
    <script>
        ...
        function delete_adv(id,iss_id, e) {
            //alert(id);
            e.preventDefault();
            var url = '{{url(getLocal()."/admin/department")}}/' + id;
