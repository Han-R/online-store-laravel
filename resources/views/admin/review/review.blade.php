@extends('layouts.cp')
@section('title') {{__('website.reviews')}}
@endsection
@section('css')
@endsection
@section('content')
            ...
            <div class="table-toolbar">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable order-column" id="toolsTable">
                    ...
                    <tr>
                        ...
                        <th> {{ucwords(__('website.comment'))}}</th>
                        <th> {{ucwords(__('website.rate'))}}</th>
                        <th> {{ucwords(__('website.approved'))}}</th>
                        <th> {{ucwords(__('website.spam'))}}</th>
                    ...
                    @forelse(@$items as $item)
                        <tr class="odd gradeX" id="tr-{{$item->id}}">
                            ...
                            <td> {{@$item->comment}}</td>
                            <td>@include('website.switchRate', array('product_rate'=>$item->rating))</td>
                            <td><input type="checkbox" {{ $item->approved?"checked":""}} disabled /></td>
                            <td><input type="checkbox" {{ $item->spam?"checked":""}} disabled /></td>
                            <td class="center">{{$item->created_at->toDateString()}}</td>
                            <td>
                                <div class="btn-group btn-action">

                                    <a href="{{url(getLocal().'/admin/review/'.$item->id.'/edit')}}"
...
@section('script')
    <script>
        function delete_adv(id,iss_id, e) {
            ...
            console.log(id);
            console.log(iss_id);
            var url = '{{url(getLocal()."/admin/review")}}/' + id;
