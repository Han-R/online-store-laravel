@extends('layouts.cp')
@section('title') {{ucwords(__('common.users'))}}
@endsection
@section('css')
@endsection
@section('content')
                ...
                <div class="row">
                    <div class="col-sm-9">
                        <div class="btn-group">
                            ...
                            <button class="btn sbold blue event" id="prohibited">{{__('common.prohibitive')}}
                                <i class="fa fa-minus"></i>
                            </button>
                            <input type="hidden" id="urlToChangeStatus" value="{{url(getLocal()."/admin/changeStatus/users")}}">
                        </div>
                    </div>
                </div>
            ...
            <table class="table table-striped table-bordered table-hover table-checkable order-column" id="toolsTable">
                ...
                @forelse($items as $item)
                    <tr class="odd gradeX" id="tr-{{$item->id}}">
                        ...
                        <td> {{$item->mobile}}</td>
                        <td>
                            <span @if($item->status == 1) class="label label-sm label-info" @elseif($item->status == 0) class="label label-sm label-default" @else  class="label label-sm label-danger" @endif
                                 id="label-{{$item->id}}">
                                @if($item->status == 1)
                                    {{__('common.active')}}
